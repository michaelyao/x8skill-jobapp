...
function buildPrompt(snapshot: PageSnapshot, ctx: AgentContext): { system: string; user: string } {
  ...
  const system = [
    ...
    '- ...but read the "optionsSample" to see what KIND of thing that list holds, and answer in the same vocabulary. A box labelled "Type to Add Skills" whose sample runs "Accounting, Actuarial Science, Advertising, Aerospace Engineering" is an ACADEMIC DISCIPLINE list, not a programming-language list: the right answer there is "Computer Science", and "Python" returns no results at all. Match the granularity of the sample too — if the sample lists broad fields, do not answer with a specific tool or technology.',
